package org.zeith.comm3.oresregen.gen;
...
public class WorldGenMinableOR
		extends WorldGenerator
{
	...
	public WorldGenMinableOR(IBlockState state, int blockCount)
	{
		this(state, blockCount, stonePredicate);
	}

	public WorldGenMinableOR(IBlockState state, int blockCount, Predicate<IBlockState> predicate)
	{
		this.oreBlock = state;
	...
	{
		gen(worldIn, rand, position);
		return true;
	}

	public int gen(World worldIn, Random rand, BlockPos position)
	{
		int count = 0;
						...
						if(d12 * d12 + d13 * d13 < 1.0D)
						{
							for(int j2 = l; j2 <= k1; ++j2)
							{
								...
								if(d12 * d12 + d13 * d13 + d14 * d14 < 1.0D)
								{
									...
									{
										worldIn.getChunk(blockpos);
										if(worldIn.setBlockState(blockpos, this.oreBlock, 2))
											++count;
										if(count >= this.numberOfBlocks)
											return count;
									}
								}
							}
